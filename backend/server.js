...
/* 🔽 your auctionState, helpers, routes,
   socket events, finalizeAuction,
   admin_start_auction, place_bid,
   all logic remains EXACTLY SAME 🔽 */
...
app.get('/api/events/:id/matches', authenticate, async (req, res) => {
    try {
        const eventId = req.params.id;
        const matches = await Match.find({ event: eventId }).populate('homeTeam').populate('awayTeam').sort({ startTime: 1 });
        res.json(matches);
    } catch (error) { res.status(400).json({ error: error.message }); }
});
...
app.get('/api/events/:id/teams', async (req, res) => {
    try {
        const teams = await Team.find({ event: req.params.id }).populate('players').sort({ createdAt: 1 });
        res.json(teams);
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.post('/api/admin/players/:id/skip', authenticate, async (req, res) => {
    try {
        if (req.user.role !== 'Admin') return res.status(403).json({ error: 'Insufficient privileges' });
        const player = await Player.findByIdAndUpdate(req.params.id, { status: 'Skipped' }, { new: true });
        res.json(player);
    } catch (error) { res.status(400).json({ error: error.message }); }
});
...
io.on('connection', (socket) => {
    socket.on("request_sync", () => {
        if (auctionState.isActive) {
            socket.emit("auction_started", auctionState);
        }
    });
    ...
    socket.on("admin_start_auction", async (playerId) => {
        ...
        const playerEventId = (player.event && (player.event._id || player.event))?.toString();
        if (!activeEvent || !playerEventId || playerEventId !== activeEvent._id.toString()) {
            return socket.emit('error', { message: 'No active event or player does not belong to the active event. Set an event as Active first.' });
        }
        auctionState = {
            ...auctionState,
    ...
    socket.on("admin_pause_auction", () => {
        if (!socket.user || socket.user.role !== 'Admin') return;
        if (auctionState.isActive && auctionState.timerInterval) {
            clearInterval(auctionState.timerInterval);
            auctionState.timerInterval = null;
        }
    });

    socket.on("admin_resume_auction", () => {
        if (!socket.user || socket.user.role !== 'Admin') return;
        if (auctionState.isActive && !auctionState.timerInterval) {
            auctionState.timerInterval = setInterval(() => {
                if (auctionState.timer > 0) {
    ...
    socket.on("admin_skip_current", async () => {
        if (!socket.user || socket.user.role !== 'Admin') return;
    ...
    socket.on("place_bid", async ({ teamId, teamName, bidAmount }) => {
        ...
        // Concurrency: update state immediately so duplicate/same-amount bids are rejected
